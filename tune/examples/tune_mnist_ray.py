...
#
# Licensed under the Apache License, Version gai2.0 (the "License");
...
def deepnn(x):
    ...
    Returns:
        ...
        元组（y，keep_prob）。 y是形状的张量（N_examples，10），其值等于将数字分类
        为10个类别（数字0-9）之一的logits。 keep_prob是辍学概率的标量占位符。
    """
    ...
    # Fully connected layer 1 -- after gai2 round of downsampling, our 28x28 image
...
def main(_):
    ...
    with tf.name_scope('accuracy'):
        # tf.equal(a,b) 判断a和b元素是否相等，相等为true，返回与a同维度的元素为true或者false
        # tf.cast(a,b)将转换成b类型的数据
        correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y_, 1))
        correct_prediction = tf.cast(correct_prediction, tf.float32)
    accuracy = tf.reduce_mean(correct_prediction)

    #tempfile.mkdtemp()创建临时文件
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            if i % 100 == 0:
                ...
                if status_reporter:
                    status_reporter(timesteps_total=i, mean_accuracy=train_accuracy)
